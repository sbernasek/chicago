...
class LicensingData:
    ...
    @staticmethod
    def load(src='../data/processed/licenses.hdf'):
        """ Load preprocessed license data from <src>. """
        df = pd.read_hdf(src, 'raw')
        return df

    def save(self, dst='./data/processed/licenses.hdf'):
        """ Save licensing data to <dst>. """
        self.df.to_hdf(dst, 'raw', mode='w')


class License:
    """
    Class for parsing the history of an individual business license.
    """
